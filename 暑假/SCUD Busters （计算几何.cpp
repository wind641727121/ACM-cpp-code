...
int relation (Line a, Circle b)  //直线和圆
{
//0:相离1:相切 2:相交
...
int get_circle (Line u, Line v, double r1, Circle &c1, Circle &c2, Circle &c3, Circle &c4)  //和直线u、v相切 半径为r1
{
    if (parallel (u, v))
        return 0;
    Line u1 = Line (u.s + (u.e-u.s).rotate_left ().change_len (r1), u.e
...
double circle_traingle_area (Point a, Point b, Circle c)  //圆形三角形面积
{
...
//******多边形
double polygon_area (Point *p, int n)  //多边形有向面积
{
    double area = 0;
    for (int i = 1; i < n-1; i++)
    {
...
bool relation (Point q, Point *p, int n)  //点和多边形关系（凹凸都行
{
//0:外部 1:内部 2:边上 3:顶点
    for (int i = 0; i < n; i++)
    {
...
double convex_circumference (Point *p, int n)  //多边形周长
{
    double ans = 0;
    for (int i = 0; i < n; i++)
    {
...
Point centre_of_gravity (Point *p, int n)  //多边形重心
{
...
bool Compl_inside_convex(const Point & p,Point *con, int n)
{
    ...
    return cross(p-con[line-1],con[line]-con[line-1])<eps; //不在边界的话变为<-eps
}
bool get_dir (Point *p, int n)  //得到多边形的时针循序
{
//0:顺时针 1:逆时针
    double sum = 0;
    for (int i = 0; i < n; i++)
...
int main()
{
    ...
    for(num=0;;num++)
    {
        ...
        for(int j =0;j<n[num];j++)
            a[num][j].input();
        n[num]=convex_hull(a[num],b[num],n[num]);
    }
    while(cin>>ha.x)
    {
        cin>>ha.y;
        //if(ha.x==-1)break;
        for(int i =0;i<num;i++)
            if(Compl_inside_convex(ha,b[i],n[i])) {vis[i]=1;break;}
    }
    double ans=0;
    for(int i =0;i<num;i++)
    {
        if(vis[i]) ans+=fabs(polygon_area(b[i],n[i]));
    }
    printf("%.2f\n",ans);
    return 0;
}
